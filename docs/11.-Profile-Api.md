...
MollieResult<ProfileResponse> result = await client.CreateProfileAsync(profileRequest);
ProfileResponse response = result.Data;
```
...
```C#
using IProfileClient client = new ProfileClient({yourApiKey});
MollieResult<ProfileResponse> result = await client.GetProfileAsync({profileId});
...
```C#
using IProfileClient client = new ProfileClient({yourApiKey});
MollieResult<ProfileResponse> result = await client.GetCurrentProfileAsync();
ProfileResponse profileResponse = result.Data;
```
...
using IProfileClient client = new ProfileClient({yourApiKey});
MollieResult<ProfileResponse> result = await client.UpdateProfileAsync({profileId}, profileRequest);
ProfileResponse profileResponse = result.Data;
```
...
```C#
using IProfileClient client = new ProfileClient({yourApiKey});
MollieResult<PaymentMethodResponse> result = await client.EnablePaymentMethodAsync({profileId}, PaymentMethod.Ideal);
PaymentMethodResponse paymentMethodResponse = result.Data;
MollieResult disableResult = await client.DisablePaymentMethodAsync({profileId}, PaymentMethod.Ideal);
```
...
MollieResult<EnableGiftCardIssuerResponse> result = await client.EnableGiftCardIssuerAsync({profileId}, issuerToToggle);
EnableGiftCardIssuerResponse paymentMethodResponse = result.Data;
MollieResult disableResult = await client.DisableGiftCardIssuerAsync({profileId}, issuerToToggle);
```
