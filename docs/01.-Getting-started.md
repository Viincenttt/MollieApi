...
The library automatically generates a unique GUID for each request and adds it to the Idempotency-Key header. Using the resilience libraries built, you are able to automatically retry requests in case of a timeout or 5xx status code error using the following example code:
```c#
...
public static class MollieApiClientServiceExtensions {
		...
		MollieConfiguration mollieConfiguration = configuration.GetSection("Mollie").Get<MollieConfiguration>();
            	...
            	services.AddHttpClient<IPaymentClient, PaymentClient>(httpClient => new PaymentClient(mollieConfiguration.ApiKey, httpClient))
                	.AddResilienceHandler("Mollie", builder => {
				// Requests are retried three times, with exponentially increasing intervals
